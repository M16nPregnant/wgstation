...
/obj/machinery/rnd/production/proc/user_try_print_id(id, amount)
	...
	if(!(isnull(allowed_department_flags) || (D.departmental_flags & allowed_department_flags)))
		say("Warning: Printing failed: This fabricator does not have the necessary keys to decrypt design schematics. Please update the research data with the on-screen button and contact Genesis Support!") //GS13 - Nanotrasen to GATO
		return FALSE
	if(D.build_type && !(D.build_type & allowed_buildtypes))
		say("This machine does not have the necessary manipulation systems for this design. Please contact Genesis Support!") //GS13 - Nanotrasen to GATO
		return FALSE
