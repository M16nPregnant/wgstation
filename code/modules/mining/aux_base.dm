...
/obj/machinery/computer/auxillary_base/proc/set_landing_zone(turf/T, mob/user, no_restrictions)
	...
	if(!base_dock) //Not all maps have an Aux base. This object is useless in that case.
		to_chat(user, "<span class='warning'>This station is not equipped with an auxillary base. Please contact your Genesis contractor.</span>") //GS13 - Nanotrasen to GATO
		return
	if(!no_restrictions)
...
//Mining shuttles may not be created equal, so we find the map's shuttle dock and size accordingly.
	for(var/S in SSshuttle.stationary)
		...
		if(SM.shuttle_id == "mining_home" || SM.shuttle_id == "mining_away")
			...
			break
	if(!Mport)
		to_chat(user, "<span class='warning'>This station is not equipped with an appropriate mining shuttle. Please contact Genesis Support.</span>") //GS13 - Nanotrasen to GATO
		return
	...
	if(!mining_shuttle) //Not having a mining shuttle is a map issue
		to_chat(user, "<span class='warning'>No mining shuttle signal detected. Please contact Genesis Support.</span>") //GS13 - Nanotrasen to GATO
